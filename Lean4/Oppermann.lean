...
/-- Oppermann vale para todo `2 ≤ x ≤ 50`. -/
private theorem oppermann_bool_upto_50 :
    (List.range' 2 49).all oppermannHolds = true := by
  native_decide

theorem oppermann_upto_50 {x : ℕ} (hx₂ : 2 ≤ x) (hx : x ≤ 50) :
    (∃ p, x * (x - 1) < p ∧ p < x ^ 2 ∧ p.Prime) ∧
    (∃ p, x ^ 2 < p ∧ p < x * (x + 1) ∧ p.Prime) := by
  have hmem : x ∈ List.range' 2 49 := by
    rw [List.mem_range'_1]
    exact ⟨hx₂, by omega⟩
  have hbool : oppermannHolds x = true :=
    (List.all_eq_true.1 oppermann_bool_upto_50) x hmem
...
/-- Legendre vale para todo `1 ≤ n ≤ 50` (vía Oppermann hasta 51, o comprobación directa). -/
